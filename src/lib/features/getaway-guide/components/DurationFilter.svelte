<script lang="ts">
	...
	let { minDuration = null, maxDuration = null, onFilterChange, totalVideos, filteredVideos } = $props<Props>();
	...
	let maxSliderValue = $state(maxDuration ? Math.max(1, Math.floor(maxDuration / STEP)) : Math.floor(MAX_SECONDS / STEP));

	// Convert slider values to actual duration values
	const actualMinDuration = $derived(minSliderValue === 0 ? null : minSliderValue * STEP);
	const actualMaxDuration = $derived(maxSliderValue === Math.floor(MAX_SECONDS / STEP) ? null : maxSliderValue * STEP);

	// Format labels for display
	const minLabel = $derived(actualMinDuration ? formatSecondsToDuration(actualMinDuration) : 'No min');
	const maxLabel = $derived(actualMaxDuration ? formatSecondsToDuration(actualMaxDuration) : 'No max');
	...
	<div class="space-y-4">
		...
		<div class="relative">
			...
			<input
				...
				class="absolute top-0 h-2 w-full cursor-pointer appearance-none bg-transparent focus:outline-none [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-[var(--accent)] [&::-webkit-slider-thumb]:shadow-[var(--shadow)] [&::-moz-range-thumb]:h-4 [&::-moz-range-thumb]:w-4 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:border-0 [&::-moz-range-thumb]:bg-[var(--accent)]"
			/>
		...
		<div class="flex flex-wrap gap-2">
			<button
				onclick={() => { minSliderValue = 0; maxSliderValue = maxSliderSteps; }}
				class="rounded-md border border-[var(--border)] px-2 py-1 text-xs hover:shadow-[var(--shadow-sm)]"
			>
				All
			</button>
			<button
				onclick={() => { minSliderValue = 0; maxSliderValue = Math.floor(300 / STEP); }} 
				class="rounded-md border border-[var(--border)] px-2 py-1 text-xs hover:shadow-[var(--shadow-sm)]"
			>
				&lt; 5 min
			</button>
			<button
				onclick={() => { minSliderValue = Math.floor(300 / STEP); maxSliderValue = Math.floor(900 / STEP); }}
				class="rounded-md border border-[var(--border)] px-2 py-1 text-xs hover:shadow-[var(--shadow-sm)]"
			>
				5-15 min
			</button>
			<button
				onclick={() => { minSliderValue = Math.floor(900 / STEP); maxSliderValue = maxSliderSteps; }}
				class="rounded-md border border-[var(--border)] px-2 py-1 text-xs hover:shadow-[var(--shadow-sm)]"
			>
				&gt; 15 min
			</button>
